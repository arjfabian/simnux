...
class Command(SNXCommand):
    ...
    async def execute(
        ...
        delete_result = ctx.filesystem.delete_file(source_path, execution=ctx.execution_context)
        if delete_result.exit_code != ExitCode.SUCCESS:
            if not target_pre_existed:
                ctx.filesystem.delete_file(target_path, execution=ctx.execution_context)
