...
class SNXShell:
    ...
    def execute(self, raw_input: str) -> CommandResult:
        ...
        self.logger.info(
            f"[{self.session.session_id}] Executing: {raw_input}"
        )
        ...
        if not self.registry.exists(parsed.command):
            self.logger.warning(
                f"[{self.session.session_id}] Unknown command: {parsed.command}"
            )
        ...
        try:
            ...
            self.logger.info(
                f"[{self.session.session_id}] Exit code: {result.exit_code}"
            )
            ...
            self.logger.exception(
                f"[{self.session.session_id}] Execution error: {parsed.command}"
            )
